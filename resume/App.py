...
def fetch_yt_video(link):
    ydl = youtube_dl.YoutubeDL({'outtmpl': '%(title)s.%(ext)s', 'logger': youtube_dl.utils.std_headers})
    try:
        with ydl:
            result = ydl.extract_info(link, download=False, verbose=True)
        return result.get('title', None)
...
# Connect to MongoDB (replace with your web-hosted connection string)
CONNECTION_STRING = "mongodb+srv://brammy:Xn0ZF91URLO79Dzn@cluster0.mtwl1s6.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
DB_NAME = "CopiedLI"
...
def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills, courses):
    ...
    collection.insert_one(data)  # Insert document into collection
...
#    cursor.execute(table_sql)
    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'brammy' and ad_password == 'brammy123':
                st.success("Welcome Brammy!")
                # Display Data
                
                #cursor.execute('''SELECT*FROM user_data''')
                #data = cursor.fetchall()
                
                data = collection.find({})  # Fetch all documents from collection
                ...
                pipeline = [
                    {"$group": {"_id": "$Predicted_Field", "count": {"$sum": 1}}},
                    {"$project": {"Predicted_Field": "$_id", "count": 1, "_id": 0}}
                ]
                plot_data = list(collection.aggregate(pipeline))
                ...
                values = plot_data_df['Predicted_Field'].value_counts()
                print(values)

                st.subheader("**Pie-Chart for Predicted Field Recommendation**")
                #fig = px.pie(df, values=values, names=labels, title='Predicted Field according to the Skills')
                fig = px.pie(plot_data_df, values='count', names='Predicted_Field', title='Predicted Field according to the Skills')
                st.plotly_chart(fig)

                # Display the available fields in the plot_data_df DataFrame
                ...
                #values = plot_data_df['User_level'].value_counts()
                values = plot_data_df['Predicted_Field'].value_counts()

                #st.subheader("**Pie-Chart for User's Experienced Level**")
                st.subheader("**Pie-Chart for Predicted Field Recommendation**")
                #fig = px.pie(df, values=values, names=labels, title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
                #fig = px.pie(plot_data_df, values='count', names='User_level', title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
                fig = px.pie(plot_data_df, values='count', names='Predicted_Field', title='Predicted Field according to the Skills')
                st.plotly_chart(fig)

                # Check the available fields and use the appropriate one for the user level pie chart
                if 'User_level' in plot_data_df.columns:
                    labels = plot_data_df['User_level'].unique()
                    values = plot_data_df['User_level'].value_counts()
                    ...
                    st.warning("The 'User_level' field is not available in the data.")
